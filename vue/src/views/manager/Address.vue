<template>
    <div>
      ...
      <el-dialog name="地址" :visible.sync="formvisible" width="40%" :close-on-click-modal="false" destroy-on-close>
        <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
          <el-form-item prop="name" label="联系人">
            <el-input v-model="form.title" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item prop="address" label="联系人地址">
    ...
    methods: {
      ...
      load(pageNum) {  // 分页查询
        ...
        this.$request.get('/address/selectPage', {
          ...
          console.log("API 请求返回的数据：", res);  // 先查看完整的响应数据
          console.log("API 请求返回的数据：", res);  // 先查看完整的响应数据

        if (res && res.data) {
            console.log("后端返回的数据列表：", res.data.list);  // 确保 list 存在
        } else {
            console.error("后端返回的数据结构不对！");
        }

        this.tableData = res.data?.list || [];  // 确保 tableData 赋值
        console.log("前端 tableData 数据：", this.tableData); // 确保数据赋值成功
