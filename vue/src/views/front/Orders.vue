<template>
  <div style="width: 85%;margin: 10px auto; " class="card">
    <div style="margin-bottom: 15px ;display: flex" >
      ...
      <div style="flex: 1">
        <el-input placeholder="请输入商品名称" style="width: 200px;margin-right: 5px" clearable></el-input>
        <el-input placeholder="请输入订单编号" style="width: 200px;margin-right: 5px" clearable></el-input>
    ...
    <div style="margin: 10px 0">
      <el-table :data="tableData" strip>
        ...
        <el-table-column prop="user" label="下单人名称"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="300">
          <template v-slot="scope">
            ...
            <el-button v-if="scope.row.status ==='待发货'" size="mini" type="info" plain @click="changeStatus(scope.row,'待收货')">发货</el-button>
            <el-button v-if="scope.row.status ==='待收货'" size="mini" type="primary" plain @click="changeStatus(scope.row,'已完成')">收货</el-button>
...
export default {
  ...
  methods: {
    changeStatus(row, status) {
      this.$confirm('您确定操作吗？', '确认操作', {type: "warning"}).then(() => {
        this.form  = JSON.parse(JSON.stringify(row))
        this.form.status = status
        this.$request.put('/orders/update',this.form).then(response => {
