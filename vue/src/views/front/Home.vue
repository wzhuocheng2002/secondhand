  ...
  <div style="background-color:#f8f8f8;min-height: 100vh;" >
    <div class="bg">
      <div style="width:600px">
        <div style="text-align: center; color:grey;font-size: 30px; margin-bottom: 20px;">好看、好玩、好用、都在这里</div>
        <div style="display: flex;">
          <el-input size="medium" prefix-icon="el-icon-search" placeholder="请输入商品名称关键字搜索"></el-input>
          <el-button style="margin-left: 5px;"><i class="el-icon-search"></i></el-button>
        </div>
      </div>
    </div>

    <div style="width:80%; background-color: #fff; margin: 10px auto; padding: 20px; border-radius: 5px; position: relative;">
        ...
        <el-row :gutter="15">
          <el-col :span="6" v-for="item in goodsList" :key="item.id">
            <div style="margin-bottom: 20px;" class="goods-item" @click="$router.push('/front/goodsDetail?id=' + item.id)">  
              <img class="goods-img" :src="item.img" alt="" style="width: 100%; display: block;height: 260px; margin-bottom: 5px; border-radius: 5px;">
              <div style="font-size: 20px; height:40px; margin-bottom: 10px;" class="line2 goods-name">{{ item.name }}</div>
...
.goods-item:hover .goods-name {
  color: #f03793 !important; 
}
.goods-item {
  cursor: pointer;
}
.goods-item img ,goods {
  transition: all .2s;
}
</style>